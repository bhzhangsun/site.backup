...
└── scripts/
    ...
    ├── setup-nginx-site.sh    # ② 自编译 nginx + OpenSSL 3.2+ + :80 HTTP + :443 TCP/UDP (h2 + h3 共 listen)
...
① setup-deploy-user.sh
   ...
   ▼
② setup-nginx-site.sh
   │  自编译 nginx + OpenSSL 3.2+（带 QUIC 支持；Debian 11 系统 OpenSSL 1.1.1 走 apt 装不出 QUIC，必须自编译）
